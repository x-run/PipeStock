...
在庫構成比の円グラフ用。商品名の先頭トークンでカテゴリ分類し、TopN + OTHER で返す。

**カテゴリ定義:**
- category = 商品名 `name` の先頭トークン (空白 / 全角空白でsplit)
- DB には保存しない (派生値)
...
| パラメータ | 型 | 必須 | デフォルト | 説明 |
|-----------|-----|------|----------|------|
| metric | string | No | value | `value` / `qty` / `available` / `reserved` |
...
| metric | 集計式 |
|--------|--------|
| value | SUM(on_hand × unit_price) |
| qty | SUM(on_hand) |
| available | SUM(on_hand − reserved_total) ※商品単位で計算後に合算 |
| reserved | SUM(reserved_total) |
...
| フィールド | 説明 |
|-----------|------|
| metric | リクエストで指定された metric 名 |
| total | 全カテゴリの metric_value 合計 |
| breakdown[].key | カテゴリ名 (商品名の先頭トークン) / `"OTHER"` (TopN 外) |
| breakdown[].label | カテゴリ名 / `"その他"` |
| breakdown[].metric_value | そのカテゴリの集計値 |
