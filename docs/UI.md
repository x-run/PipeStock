...
#### 3.1.2 売上円グラフ (Pie Chart)

棒グラフの右横（またはサマリカードエリア）に売上円グラフを表示する。
...
│           B    その他               │
│                                     │
│  ● PIPE-001 ステンレス  ¥100,000    │
│  ● PIPE-002 銅パイプ    ¥ 50,000    │
│  ● その他               ¥ 30,000    │
└─────────────────────────────────────┘
```

**データソース:** `GET /api/v1/dashboard/sales/pie`

**要件:**
- 中央に total_yen を大きく表示
- 期間切替: preset (今月 / 3ヶ月 / 今年) + カスタム (start/end)
- TopN + OTHER でスライスを構成
- product_id=null の売上は「不明な商品」(UNKNOWN) として表示
- REFUND がある場合、refund_total_yen を total の下に薄い色で表示
- limit のデフォルトは 10
