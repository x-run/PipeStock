...
| 3 | 入出庫操作 | `/transactions` | 実装済 | 商品検索 + 入出庫フォーム + プレビュー + 履歴 |
| 4 | 商品一覧 | `/products` | 未実装 | 商品マスタの管理 |
| 5 | 商品登録/編集 | `/products/new`, `/products/:id/edit` | 未実装 | 商品の作成・更新 |
| 6 | 商品詳細 | `/products/:id` | 未実装 | 商品情報 + Tx 履歴 |
| 7 | Tx 登録 | `/products/:id/transactions/new` | 未実装 | 入庫・出庫・引当・補正の記録 |
...
### 3.1.4 入出庫操作ページ (`/transactions`) — 実装済

商品検索から入出庫実行までを1画面で完結できる専用ページ。リッチなフォームUIと操作プレビュー機能を提供。
...
**4) 理由/用途選択**
- 入庫: 自動的に "PURCHASE" を設定（フォーム表示なし、note表示）
- 出庫: ドロップダウンで用途選択必須（SHIP/MOVE/SCRAP/SAMPLE/OTHER）

**5) 操作プレビュー（サイドバー）**
- 実行前に変更後のAvailableを表示
- 「80 → 130」のような差分表示
- 在庫不足の場合は警告表示
...
5. プレビューで確認
6. 実行ボタン押下
7. API送信 → 成功時にトースト表示 + フォームリセット + 履歴追加

**バリデーション:**
- 商品未選択時: エラー「商品を選択してください」
- 数量不正時: エラー「数量は1以上の整数を入力してください」
- 出庫時に用途未選択: エラー「用途を選択してください」
- 在庫不足時（API 409）: エラー「在庫が足りません」

**API:**
- 商品検索: `GET /api/v1/stock?q={query}&per_page=10`
- 入出庫実行: `POST /api/v1/products/{id}/transactions`
  - body: `{ type: 'IN'|'OUT', qty, reason }`

**実装コンポーネント:**
- `TransactionsPage` — 入出庫操作専用ページ
